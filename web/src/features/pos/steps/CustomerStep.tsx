...
export function CustomerStep() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      <h2 className="text-xl font-bold text-brand-dark">Step 1: Customer Selection</h2>
      <p className="mb-4 text-sm text-muted">
        Enter customer info or pick a match below. Leave as-is for a walk-in.
      </p>

      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <input className={field} placeholder="Name *" value={form.name} autoFocus onChange={(e) => set('name', e.target.value)} />
        <input className={field} placeholder="Mobile Phone" value={form.phone} onChange={(e) => set('phone', e.target.value)} />
      ...
      <div className="mb-2 text-sm font-semibold text-muted">Matching customers</div>
      <div className="mb-4 max-h-56 overflow-auto rounded-xl border border-line bg-white">
        {form.name.trim().length < 2 && (
          <p className="p-4 text-sm text-faint">Start typing a name to search…</p>
        )}
        {results.isFetching && <p className="p-4 text-sm text-muted">Searching…</p>}
        ...
        <ul className="divide-y divide-line/40">
          {(results.data ?? []).map((c) => (
            <li key={c.id}>
              <button
                onClick={() => pick(c)}
                className={`flex w-full items-center justify-between px-4 py-2.5 text-left hover:bg-brand-bg ${
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          >
            {state.busy ? 'Saving…' : 'Continue with Customer →'}
          </button>
        </div>
      </div>
    </div>
